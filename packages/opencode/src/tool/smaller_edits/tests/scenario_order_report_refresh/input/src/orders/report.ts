...
function detailedStatusText(order: Order) {
...
function summaryStatusText(order: Order) {
  if (order.status === "draft") return "Draft"
  if (order.status === "paid") return "Paid"
  if (order.status === "overdue") return "Overdue"
  if (order.status === "hold") return "Hold"
...
function renderOrderRow(order: Order) {
  const status = detailedStatusText(order)
  return [
...
export function renderExpandedSummary(orders: Order[]) {
  ...
  for (const order of orders) {
    lines.push(`- ${order.id}: ${summaryStatusText(order)} for ${order.customer}`)
  }
