...
function headingForSection(section: RunbookSection) {
...
function summarySection(section: RunbookSection) {
  if (section === "prepare") return "Prepare"
  if (section === "verify") return "Verify"
  if (section === "rollback") return "Rollback"
  return "Followup"
...
export function renderOutline(steps: RunbookStep[]) {
  ...
  for (const step of steps) {
    lines.push(`${step.id} ${headingForSection(step.section)} :: ${step.title}`)
...
export function renderSectionSummary(steps: RunbookStep[]) {
  return steps.map((step) => `${summarySection(step.section)} -> ${step.title}`).join("\n")
}
