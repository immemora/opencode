...
function renderItem(item: DigestItem) {
  return [
    `- ${item.id} ${item.title}`,
    `  owner: ${item.owner}`,
    `  bucket: ${bucketLabel(item.bucket)} [freshness=${freshnessLabel(item.hoursStale)}]`,
...
export function renderOwnerSummary(items: DigestItem[]) {
  ...
  for (const item of items) {
    lines.push(
      `${item.owner}: ${bucketLabel(item.bucket)} -> ${item.title} [freshness=${freshnessLabel(item.hoursStale)}]`,
    )
  }
  return lines.join("\n")
}
...
export const seedItems: DigestItem[] = [
  ...
  },
  {
    ...
    notes: ["rolled out", "support briefed"],
  },
]
